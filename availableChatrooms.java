	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			for (int i=0; i < classes.size(); i++) {
				...
				if(!isInstructor) {
					out.println("<button name=\"recipient\" type=\"submit\" value=\"" + classes.get(i).getInstructor().getUserId() + "\">" + classes.get(i).getInstructor().getFullName() + "</button> <br>");
				}
				for (int j=0; j < studentList.size(); j++) {
					if (!studentList.get(j).getUserId().equals(senderId)) {
						out.println("<button name=\"recipient\" type=\"submit\" value=\"" + studentList.get(j).getUserId() + "\">" + studentList.get(j).getFullName() + "</button> <br>");
					} 
				}
			...
			//session.setAttribute("recipientId", request.getParameter("recipient")); // Set Attribute
			...
			//request.setAttribute("recipientId", request.getParameter("recipient").toString());
			//RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/chatWindow");
